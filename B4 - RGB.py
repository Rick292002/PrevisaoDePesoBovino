...
# Configurações
DATASET_PATH = "dataset/gado/Vector/B4/Side/data/images"
NEW_IMAGE_PATH = "Foto nova/imagens/side.jpg"  # Diretório da nova imagem sem rótulo
RESULTS_FILE = "B4 - RGB - Side.xlsx"
BATCH_SIZE = 16
EPOCHS = 10
K_FOLDS = 10
...
def parse_filename(filename):
    match = re.match(r"(\d+)_b\d+-\d+_([sr])_(\d+)_([MF])", filename)
    if match:
        return {
...
# Modelo CNN (ResNet50 pré-treinada)
...
# Verificar se há pesos salvos e carregar
MODEL_PATH = "B4 - Side - RGB.pth"
OPTIMIZER_PATH = "optimizer-b4-rgb-side.pth"
...
def mostrar_exemplos():
    ...
    with torch.no_grad():
        for idx in amostra_indices:
            ...
            plt.title(
                f"ID: {img_id}, Peso Real: {real_weight:.2f} kg, Previsto: {predicted_weight:.2f} kg\nPosição: {position}, Sexo: {sex}")
            plt.axis('off')
            plt.show()
...
dot.render("Topologia B4 RGB Side", format="png", cleanup=True)  # Salva como PNG
