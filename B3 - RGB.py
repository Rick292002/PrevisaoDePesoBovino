...
# Configurações
DATASET_PATH = "dataset/gado/Vector/B3/Rear/data/images"
NEW_IMAGE_PATH = "Foto nova/imagens/rear.jpg"
RESULTS_FILE = "B3 - RGB - Rear.xlsx"
#B2 usou 194 imagens, B3 tem 2.603 imagens. Como o batch_size está 16, ele pode estar dificultando a convergência
BATCH_SIZE = 16
EPOCHS = 10
K_FOLDS = 10
LEARNING_RATE = 0.001  # Menor taxa de aprendizado para melhor precisão
...
def parse_filename(filename):
    match = re.match(r"(\d+)_([sr])_(\d+)_([MF])", filename)
    if match:
        return {
            "id": float(match.group(1)),  # ID como número
            "position": "Side" if match.group(2) == "s" else "Rear",  # Posição como "Side" ou "Rear"
            "weight": float(match.group(3)),  # Peso real como número
            "sex": match.group(4)  # Sexo, que pode ser "M" ou "F"
        }
    return None
...
class CattleDataset(Dataset):
    def __init__(self, image_dir, transform=None):
        ...
        print(f"Imagens encontradas: {len(self.images)}")  # Adiciona o print para ver quantas imagens estão no diretório
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normalização padrão do ResNet
])
...
MODEL_PATH = "B3 - Rear - RGB.pth"
OPTIMIZER_PATH = "optimizer-b3-rgb-rear.pth"
...
def train_fold(train_loader, model, criterion, optimizer):
    ...
    for batch in train_loader:
        ...
        # Garantir que `labels` seja uma lista de dicionários (se necessário)
        if isinstance(labels, tuple) and isinstance(labels[0], list):
            labels = labels[0]
        ...
        if not isinstance(labels, list) or not all(isinstance(label, dict) for label in labels):
            raise ValueError(f"Formato inesperado de labels: {labels}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(data_indices)):
    ...
    def collate_fn(batch):
        images, metadata = zip(*batch)  # Descompacta a batch em imagens e metadados
        images = [img if isinstance(img, torch.Tensor) else transforms.ToTensor()(img) for img in images]  
        images = torch.stack(images)  # Converte lista de tensores para um único tensor
        return images.to(DEVICE), list(metadata)  # Retorna metadados como lista
...
dot.render("Topologia B3 RGB Rear", format="png", cleanup=True)  # Salva como PNG
